...
const css = {
  code: ".daygrid90.svelte-1p2wps9.svelte-1p2wps9{-ms-overflow-style:none;scrollbar-width:none}.daygrid90.svelte-1p2wps9.svelte-1p2wps9::-webkit-scrollbar{display:none}.oneline.svelte-1p2wps9.svelte-1p2wps9{transition:transform 0.1s ease-in;cursor:pointer}.oneline.svelte-1p2wps9.svelte-1p2wps9:hover{transform:scaleY(1.2)}.oneline.svelte-1p2wps9:hover+.show-hover.svelte-1p2wps9{display:block !important}.show-hover.svelte-1p2wps9.svelte-1p2wps9{display:none;top:40px;padding:0px;text-align:left}.today-sq+.hiddenx.svelte-1p2wps9 .message.svelte-1p2wps9{position:absolute;white-space:nowrap}.today-sq.svelte-1p2wps9+.hiddenx.svelte-1p2wps9{visibility:hidden;z-index:30}.today-sq.svelte-1p2wps9:hover+.hiddenx.svelte-1p2wps9{visibility:visible}.today-sq.svelte-1p2wps9.svelte-1p2wps9:hover{box-shadow:rgba(50, 50, 105, 0.15) 0px 2px 5px 0px,\n            rgba(0, 0, 0, 0.05) 0px 1px 1px 0px;opacity:0.75;transition:all 0.1s ease-in;cursor:pointer}.today-sq.svelte-1p2wps9.svelte-1p2wps9{position:relative;z-index:0}",
  map: null
};
const Monitor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="grid grid-cols-12 gap-4 w-full monitor pb-4 md:w-[890px]">${monitor.embed === void 0 ? `<div class="col-span-12 md:col-span-4"><div class="pt-1"><div class="scroll-m-20 text-2xl font-semibold tracking-tight">${monitor.image ? `<img${add_attribute("src", monitor.image, 0)} class="w-6 h-6 inline"${add_attribute("alt", monitor.name, 0)} srcset="">` : ``} <span>${escape(monitor.name)}</span> <br> ${monitor.description ? `${validate_component(Root, "Popover.Root").$$render($$result, {}, {}, {
      default: () => {
        return `${validate_component(Trigger, "Popover.Trigger").$$render($$result, {}, {}, {
          default: () => {
            return `<span class="${"pt-0 pl-1 menu-monitor pr-0 pb-0 " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1p2wps9"}">${validate_component(Info$1, "Info").$$render($$result, { size: 12, class: "text-muted-foreground" }, {}, {})}</span>`;
          }
      ...
      default: () => {
        return `${validate_component(Trigger, "Popover.Trigger").$$render($$result, {}, {}, {
          default: () => {
            return `<span class="${"pt-0 pl-1 pb-0 menu-monitor pr-0 " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1p2wps9"}">${validate_component(Share2, "Share2").$$render($$result, { size: 12, class: "text-muted-foreground" }, {}, {})}</span>`;
          }
          ...
          {
            default: () => {
              return `<h2 class="mb-1 text-lg font-semibold px-2">${escape(l(lang, "monitor.share"))}</h2> <p class="pl-2 mb-2 text-muted-foreground text-sm">${escape(l(lang, "monitor.share_desc"))}</p> ${validate_component(Button, "Button").$$render($$result, { class: "ml-2", variant: "secondary" }, {}, {
                default: () => {
                  return `${`${validate_component(Link$1, "Link").$$render($$result, { class: "inline mr-2", size: 12 }, {}, {})} <span class="text-sm font-medium">${escape(l(lang, "monitor.cp_link"))}</span>`}`;
                }
              })} <h2 class="mb-2 mt-4 text-lg font-semibold px-2">${escape(l(lang, "monitor.embed"))}</h2> <p class="pl-2 mb-2 text-muted-foreground text-sm">${escape(l(lang, "monitor.embed_desc"))}</p> <div class="grid grid-cols-2 gap-2"><div class="col-span-1 pl-4"><h3 class="text-sm mb-2 text-muted-foreground">${escape(l(lang, "monitor.theme"))}</h3> ${validate_component(Radio_group, "RadioGroup.Root").$$render(
                $$result,
                { value: theme },
                {
                  value: ($$value) => {
              ...
              )}</div> <div class="col-span-1 pl-2"><h3 class="text-sm mb-2 text-muted-foreground">${escape(l(lang, "monitor.mode"))}</h3> ${validate_component(Radio_group, "RadioGroup.Root").$$render(
                $$result,
                { value: embedType },
                {
                  value: ($$value) => {
                ...
                {
                  default: () => {
                    return `${`${validate_component(Code$1, "Code").$$render($$result, { class: "inline mr-2", size: 12 }, {}, {})} <span class="text-sm font-medium">${escape(l(lang, "monitor.cp_code"))}</span>`}`;
                  }
                }
              )} <h2 class="mb-2 mt-2 text-lg font-semibold px-2">${escape(l(lang, "monitor.badge"))}</h2> <p class="pl-2 mb-2 text-muted-foreground text-sm">${escape(l(lang, "monitor.badge_desc"))}</p> ${validate_component(Button, "Button").$$render(
                $$result,
                {
                ...
                {
                  default: () => {
                    return `${`${validate_component(TrendingUp, "TrendingUp").$$render($$result, { class: "inline mr-2", size: 12 }, {}, {})} <span class="text-sm font-medium">${escape(l(lang, "monitor.status"))} ${escape(l(lang, "monitor.badge"))}</span>`}`;
                  }
                }
                ...
                {
                  default: () => {
                    return `${`${validate_component(Percent$1, "Percent").$$render($$result, { class: "inline mr-2", size: 12 }, {}, {})} <span class="text-sm font-medium">${escape(l(lang, "monitor.uptime"))} ${escape(l(lang, "monitor.badge"))}</span>`}`;
                  }
                }
              )}`;
            }
    ...
    })}</div></div> <div class=""><div class="grid grid-cols-2 gap-0"><div class="col-span-1 -mt-2"><a href="${"/incident/" + escape(monitor.folderName, true) + "#past_incident"}" class="${"pt-0 pl-0 pb-0 text-indigo-500 text-left " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1p2wps9"}">${escape(l(lang, "root.recent_incidents"))} ${validate_component(ArrowRight, "ArrowRight").$$render($$result, { size: 16 }, {}, {})}</a></div></div></div></div>` : ``} <div class="${"col-span-12 md:w-[546px] " + escape(
      ...
      true
    ) + " md:col-span-4 text-right h-[32px]"}">${_90Day[todayDD] ? `<div class="${"text-api-up text-sm truncate font-semibold mt-[4px] text-" + escape(_90Day[todayDD].cssClass, true) + " svelte-1p2wps9"}"${add_attribute("title", _90Day[todayDD].message, 0)}>${escape(summaryTime(lang, _90Day[todayDD].message))}</div>` : ``}</div></div> ${`<div class="chart-status relative mt-1 col-span-12"><div class="flex overflow-x-auto daygrid90 overflow-y-hidden py-1 svelte-1p2wps9">${each(Object.entries(_90Day), ([ts, bar]) => {
      return `<div class="h-[30px] w-[6px] rounded-sm oneline svelte-1p2wps9"><div class="${"h-[30px] bg-" + escape(bar.cssClass, true) + " w-[4px] rounded-sm mr-[2px] svelte-1p2wps9"}"></div></div> <div class="absolute show-hover text-sm bg-background svelte-1p2wps9"><div class="${"text-" + escape(bar.cssClass, true) + " font-semibold svelte-1p2wps9"}">● ${escape(n(lang, new Date(bar.timestamp * 1e3).toLocaleDateString()))} ${escape(summaryTime(lang, bar.message))}</div> </div>`;
    })}</div></div>`}</div></div> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
export { Monitor as M };
//# sourceMappingURL=monitor-5e0b1df7.js.map
