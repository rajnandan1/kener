...
const Monitor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  for (let i = 0; i <= minuteFromMidnightTillNow; i++) {
    moment().startOf("day").add(i, "minutes").format("YYYY-MM-DD HH:mm:00");
  }
  ...
  return `<section class="mx-auto backdrop-blur-[2px] mb-8 flex w-full max-w-[770px] flex-1 flex-col items-start justify-center">${validate_component(Card, "Card.Root").$$render($$result, { class: "w-full" }, {}, {
    default: () => {
      return `${validate_component(Card_header, "Card.Header").$$render($$result, {}, {}, {
        default: () => {
          return `<div class="grid grid-cols-3 gap-4"><div class="${"col-span-2 relative " + escape(!!monitor.image ? "pl-11" : "", true)}">${monitor.image ? `<img${add_attribute("src", monitor.image, 0)} class="w-8 h-8 left-0 top-[1px] absolute" alt="" srcset="">` : ``} ${validate_component(Card_title, "Card.Title").$$render($$result, {}, {}, {
            default: () => {
          ...
          })}` : ``}</div> <div class="col-span-1 text-right">${monitor.hasActiveIncident ? `<a href="${"incident/" + escape(monitor.folderName, true)}"><a href="${"/incident/" + escape(monitor.folderName, true) + "#active_incident"}" class="${escape(buttonVariants({ variant: "outline" }), true) + " relative"}"><span class="animate-ping absolute -right-[2px] -top-[2px] w-[8px] h-[8px] inline-flex rounded-full h-3 w-3 bg-red-500 opacity-75"></span>
                            Ongoing Incident</a></a>` : `<div class="float-right mt-[8px]"><picture><source srcset="https://fonts.gstatic.com/s/e/notoemoji/latest/1f4af/512.webp" class="float-right" type="image/webp"> <img src="https://fonts.gstatic.com/s/e/notoemoji/latest/1f4af/512.gif" alt="💯" width="32" height="32"></picture></div>`}</div></div>`;
        }
        ...
        default: () => {
          return `<div class="grid grid-cols-4 gap-4 mb-4"><div class="col-span-3 text-left">${validate_component(Button, "Button").$$render(
            $$result,
            {
          ...
          )}</div> <div class="col-span-1 text-right"><a href="${"/incident/" + escape(monitor.folderName, true) + "#past_incident"}"${add_attribute("class", buttonVariants({ variant: "ghost" }), 0)}>Past Incidents ${validate_component(ArrowRight, "ArrowRight").$$render($$result, { size: 16 }, {}, {})}</a></div></div> ${`<div>${`${validate_component(Skeleton, "Skeleton").$$render($$result, { class: "w-[720px] h-[40px] mt-4" }, {}, {})}`}</div>`}`;
        }
      })}`;
    }
  })}</section>`;
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${data.site.hero ? `<section class="mx-auto flex w-full max-w-4xl flex-1 flex-col items-start justify-center"><div class="mx-auto max-w-screen-xl px-4 pt-32 pb-16 lg:flex lg:items-center"><div class="mx-auto max-w-3xl text-center blurry-bg">${data.site.hero.image ? `<img${add_attribute("src", data.site.hero.image, 0)} class="h-16 w-16 m-auto" alt="" srcset="">` : ``} ${data.site.hero.title ? `<h1 class="bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 bg-clip-text text-5xl font-extrabold text-transparent leading-snug">${escape(data.site.hero.title)}</h1>` : ``} ${data.site.hero.subtitle ? `<p class="mx-auto mt-4 max-w-xl sm:text-xl">${escape(data.site.hero.subtitle)}</p>` : ``}</div></div></section>` : ``} ${each(data.monitors, (monitor) => {
    return `${validate_component(Monitor, "Monitor").$$render($$result, { monitor, github: data.site.github }, {}, {})}`;
  })}`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-caadab88.js.map
