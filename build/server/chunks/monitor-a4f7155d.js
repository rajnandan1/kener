...
import { h as setCtx$2, i as getCtx$1, j as setCtx$1, k as getCtx, l as setItemCtx, m as getRadioIndicator, n as builder, o as getAttrs$3, p as getAttrs$2, q as getAttrs$1, r as addMeltEventListener } from './ctx-a5738360.js';
import { d as derived } from './index2-ef0fcb8d.js';
import { c as cn, b as createDispatcher, f as flyAndScale, I as Icon$1 } from './Icon-c4d21504.js';
...
const css = {
  code: ".daygrid90.svelte-1c5idpg{-ms-overflow-style:none;scrollbar-width:none}.daygrid90.svelte-1c5idpg::-webkit-scrollbar{display:none}",
  map: null
};
const Monitor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="grid grid-cols-12 gap-4 monitor pb-4">${monitor.embed === void 0 ? `<div class="col-span-12 md:col-span-4"><div class="pt-1"><div class="scroll-m-20 text-2xl font-semibold tracking-tight">${monitor.image ? `<img${add_attribute("src", monitor.image, 0)} class="w-6 h-6 inline"${add_attribute("alt", monitor.name, 0)} srcset="">` : ``} <span>${escape(monitor.name)}</span> <br> ${monitor.description ? `${validate_component(Root, "Popover.Root").$$render($$result, {}, {}, {
      default: () => {
        return `${validate_component(Trigger, "Popover.Trigger").$$render($$result, {}, {}, {
          default: () => {
            return `<span class="${"pt-0 pl-1 menu-monitor pr-0 pb-0 " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1c5idpg"}">${validate_component(Info$1, "Info").$$render($$result, { size: 12, class: "text-muted-foreground" }, {}, {})}</span>`;
          }
      ...
      default: () => {
        return `${validate_component(Trigger, "Popover.Trigger").$$render($$result, {}, {}, {
          default: () => {
            return `<span class="${"pt-0 pl-1 pb-0 menu-monitor pr-0 " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1c5idpg"}">${validate_component(Share2, "Share2").$$render($$result, { size: 12, class: "text-muted-foreground" }, {}, {})}</span>`;
          }
        })} ${validate_component(Popover_content, "Popover.Content").$$render($$result, { class: " pl-1 pr-1 pb-1 w-[375px]" }, {}, {
          default: () => {
    ...
    })}</div></div> <div class=""><div class="grid grid-cols-2 gap-0"><div class="col-span-1 -mt-2"><a href="${"/incident/" + escape(monitor.folderName, true) + "#past_incident"}" class="${"pt-0 pl-0 pb-0 text-indigo-500 text-left " + escape(buttonVariants({ variant: "link" }), true) + " svelte-1c5idpg"}">Recent Incidents ${validate_component(ArrowRight, "ArrowRight").$$render($$result, { size: 16 }, {}, {})}</a></div></div></div></div>` : ``} <div class="${"col-span-12 " + escape(monitor.embed === void 0 ? "md:col-span-8" : "", true) + " pt-2"}"><div class="grid grid-cols-12"><div class="${escape(
      ...
      true
    ) + " md:col-span-4 text-right h-[32px]"}">${_90Day[todayDD] ? `<div class="${"text-api-up " + escape(monitor.embed === void 0 ? "md:pr-6" : "", true) + " text-sm font-semibold mt-[4px] text-" + escape(_90Day[todayDD].cssClass, true) + " svelte-1c5idpg"}">${escape(_90Day[todayDD].message)}</div>` : ``}</div></div> <div class="grid grid-cols-12">${`<div class="chart-status relative mt-1 col-span-12"><div class="flex overflow-x-auto daygrid90 overflow-y-hidden svelte-1c5idpg">${each(Object.entries(_90Day), ([ts, bar]) => {
      return `<div class="h-[30px] w-[6px] rounded-sm oneline"><div class="${"h-[30px] bg-" + escape(bar.cssClass, true) + " w-[4px] rounded-sm mr-[2px] svelte-1c5idpg"}"></div></div> <div class="absolute show-hover text-sm bg-background"><div class="${"text-" + escape(bar.cssClass, true) + " font-semibold svelte-1c5idpg"}">${bar.message != "No Data" ? `● ${escape(new Date(bar.timestamp * 1e3).toLocaleDateString())} ${escape(bar.message)}` : `● ${escape(new Date(bar.timestamp * 1e3).toLocaleDateString())} ${escape(bar.message)}`}</div> </div>`;
    })}</div></div>`}</div></div> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
export { Monitor as M };
//# sourceMappingURL=monitor-a4f7155d.js.map
