...
import type { MonitorRecord, MonitorAlert, TriggerRecord } from "./types/db.js";
import type { AlertData, SiteDataForNotification } from "./notification/variables.js";
...
interface TriggerObj {
  down_trigger?: AlertConfig;
  degraded_trigger?: AlertConfig;
}
...
async function createJSONCommonAlert(
  ...
  let siteURL = siteData.siteURL as string;
  ...
  let details = {
    metric: monitor.name,
    current_value: String(alert.health_checks),
...
async function createNewIncident(
  monitor: MonitorRecord,
  alert: MonitorAlert,
  commonData: AlertData,
): Promise<{ incident_id: number } | undefined> {
...
async function closeIncident(alert: MonitorAlert, comment: string): Promise<void> {
  //check if incident is already resolved
  let incident = await GetIncidentByIDDashboard({
    incident_id: alert.incident_number,
  });
  if (!incident || incident.status === CLOSED || incident.state === RESOLVED) {
    console.log("Incident is already resolved or not found");
    return;
  }

  let incident_id = alert.incident_number;
  await AddIncidentComment(incident_id, comment, RESOLVED, moment(alert.updated_at).unix());
}

function createClosureComment(alert: MonitorAlert, _commonJSON: AlertData): string {
...
async function alerting(m: MonitorInput): Promise<void> {
  let monitor = await db.getMonitorByTag(m.tag);
  if (!monitor) {
    console.log(`Monitor ${m.tag} not found`);
    return;
  }
  let siteData = (await GetAllSiteData()) as unknown as SiteDataForNotification;
  ...
  for (const key in triggerObj) {
    if (Object.prototype.hasOwnProperty.call(triggerObj, key)) {
      const alertConfig = triggerObj[key as keyof TriggerObj];
      ...
      if (alertingChannels.length > 0) {
        for (let i = 0; i < alertingChannels.length; i++) {
          ...
          if (!trigger.trigger_type || !trigger.trigger_meta) {
            console.error(`Trigger ${triggerID} is missing trigger_type or trigger_meta`);
            continue;
          }
          const notificationClient = new notification(
            { trigger_type: trigger.trigger_type, trigger_meta: trigger.trigger_meta },
            siteData,
            monitor,
          );
      ...
      let activeAlert: MonitorAlert | null | undefined = null;
      if (alertExists) {
      ...
      } else if (isAffected && alertExists && activeAlert) {
