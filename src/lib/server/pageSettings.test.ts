...
describe("pageSettings — monitor_latency_display", () => {
  it("defaults to avg only", () => {
    expect(getDefaultPageSettings().monitor_latency_display).toEqual({ avg: true, min: false, max: false });
  });
  ...
  it("merges a partial patch over defaults", () => {
    ...
    expect(merged.monitor_latency_display).toEqual({ avg: true, min: true, max: false });
  });
  ...
  it("reads stored flags and fills missing ones from defaults", () => {
    ...
    expect(toApiPageSettings(json).monitor_latency_display).toEqual({ avg: false, min: false, max: true });
  });
  ...
  it("ignores non-boolean stored flags (falls back to defaults)", () => {
    ...
    expect(toApiPageSettings(json).monitor_latency_display).toEqual({ avg: true, min: false, max: false });
  });
