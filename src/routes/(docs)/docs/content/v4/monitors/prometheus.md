...
- **Authentication.** Add an `Authorization` header under `headers`; use `$SECRET_NAME` to substitute an environment variable. Secret substitution applies to the URL and header values only — never to the query. If `url` uses plain `http://`, substituted secrets are sent in cleartext — prefer `https://` unless the server is on a trusted network.
