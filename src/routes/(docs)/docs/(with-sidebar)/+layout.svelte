...
<div class="bg-background text-foreground min-h-screen">
  ...
  <div class="mx-auto flex max-w-[1400px] pt-24">
    ...
    {#if isMobileMenuOpen}
      <div
        class="fixed inset-0 top-24 z-[35] bg-black/50 lg:hidden"
    ...
    <!-- Sidebar -->
    <aside
      class="bg-background border-border fixed top-24 bottom-0 left-0 z-40 w-[280px] -translate-x-full overflow-y-auto border-r transition-transform duration-300 ease-in-out lg:translate-x-0"
    ...
    <!-- Main content -->
    <div class="min-w-0 flex-1 p-6 px-8 lg:ml-[280px] lg:p-8 lg:px-12">
      {@render children()}
    </div>
  </div>
</div>
